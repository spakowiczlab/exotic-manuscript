...
First, we will re-calculation all dates to be in terms of days from diagnosis.
```{r}
tho.times <- tho %>%
  mutate(Stage  = ifelse(Cancer.Stage.at.Collection %in% c("88", "99", "undeterm.") , 
                         "Unknown", Cancer.Stage.at.Collection)) %>%
...
Now let's cluster the patients.
...
And now we can create a timeline for each patient, with them ordered by the clustering.
...
tho.times %>%
  ...
  ggplot(aes(x = days.diag.to.ev, y = fct_relevel(pat.id, pat.ord))) +
  geom_path(alpha = .75, aes(color = Stage)) +
  geom_point(aes(shape = Event.type)) +
