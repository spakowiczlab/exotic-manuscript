...
# 
# write.csv(loop.output, "../../data/survival-microbe-output.csv", row.names = F)
loop.output <- read.csv("../../data/survival-microbe-output.csv", stringsAsFactors = F)
```
...
loop.output %>%
  ...
  ggplot(aes(y = cancer, x = Taxa, fill = pvalue)) +
  geom_tile() +
  ...
  ggsave("../../figures/AACR/survival-significance.emf", width = 35, height = 4)
```


# Fusobacteria prevalence

```{r}
test <- loop.input %>%
  mutate(Fusobacteria = as.factor(ifelse(phylum_Fusobacteria == 1, "Present", "Absent")),
         Cancer = as.factor(Cancer)) %>%
  select(Cancer, Fusobacteria)
  
test <- table(test$Cancer, test$Fusobacteria)
mosaicplot(test, main = "Fusobacteria presence across cancers")
```

```{r}
png("../../figures/AACR/mosaicplot_fuso-presence.png")
mosaicplot(test, main = "Fusobacteria presence across cancers")
dev.off()
```
...
```{r}
#Quick data frame potentially useful for identifying interesting curves
sig.combos <- loop.output %>%
  filter(pvalue < 0.05) %>%
  mutate(canc.sig = 1) %>%
  spread(key = "cancer", value = 'canc.sig')
```
...
example.fit <- surv_fit(Surv(days, vitalstatus) ~ genus_Malassezia, data = loop.tmp)
example.plot <- ggsurvplot(example.fit, pval = T, risk.table = T, palette = c("royalblue", "firebrick"))
```

```{r}
emf("../../figures/example-survival-curve.emf")
print(print(example.plot))
dev.off()
```