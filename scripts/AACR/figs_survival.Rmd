...
loop.output %>%
  ...
  ggplot(aes(x = cancer, y = Taxa, fill = pvalue)) +
  geom_tile() +
  geom_text(aes(label = ifelse(hazard.ratio< 1, "+", "")), size = 1) +
  facet_col(vars(fct_relevel(capitalize(Taxa.Level), capitalize(taxalevels))),
  ...
  ggsave("../../figures/AACR/survival-significance.png", width = 5, height = 25)
```
