...
tabMat.v2 <- clindat.form %>%
  mutate(percent.male = ifelse(Gender == "Male", 1, 0),
         patnum = 1) %>%
  group_by(Cancer) %>%
  summarise(dem.Percent.Male = mean(percent.male),
...
tab2.all <- tab2.cancers %>%
  ...
  select(Taxa, DDX58 , MB21D1 , Survival.significant , Immune.correlation) 

tab2.all[, -1] <- bind_rows(lapply(tab2.all[,-1], function(x) ifelse(is.na(x), 0, x)))

tab2.all$times.sig <- rowSums(tab2.all[, -1])
