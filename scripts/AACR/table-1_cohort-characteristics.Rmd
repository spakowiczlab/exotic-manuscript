...
clindat.form %>%
  ...
  separate(Chemo.Drug.Name, into = c('testChem'), sep = " ") %>%
  group_by(testChem) %>%
  tally() %>%
  arrange(desc(n))
...
clindat.form %>%
  ...
  separate(I.O.Drug.Name, into = c('testIO'), sep = " ") %>%
  group_by(testIO) %>%
  tally() %>%
  arrange(desc(n))
```
...
clindat.tabnames <- clindat.form %>%
  mutate(Stage  = Cancer.Stage.at.Collection,
         ...
         Chemotherapy = ifelse(is.na(Chemo.Drug.Name), "none", tolower(Chemo.Drug.Name)),
