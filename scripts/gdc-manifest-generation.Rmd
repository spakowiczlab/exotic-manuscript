...
# write.csv(grab.uids, "../data/tcga-gdc-manifest_date.csv", row.names = F)
```
...
```{r}
# grab.uids <- read.csv("../data/tcga-gdc-manifest_date.csv")
...
for(i in nrow(grab.uids)){
  ...
  writeLines(c(paste("#PBS -N exoTCC_TCGA-validate", u, sep = "_"),
               ...
               paste0("Rscript -e 'GenomicDataCommons::fnames = gdcdata(", u, ", progress=FALSE)'"), # Need to check                                                           if BAMs are maintained in the cache outside of R sessions
               paste0("$testpaired=(samtools view -c -f 1 ", gdccache, "/", u, "/", bnam, ")"), # count number of                                                                                                        paired reads
               paste0("if[ $testpaired -eq 0 ];then rm ", gdccache, "/", u, "/", bnam,";fi"), # Causes rest of script               to fail if their are no paired reads, so we will be left only with data for the BAMs we want in the end 
               "",
