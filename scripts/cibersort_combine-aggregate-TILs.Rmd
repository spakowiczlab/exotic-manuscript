...
files <- list.files(path = "../data/split-deconvolved-aggcuff_absolute/",
                    pattern = "^CIBER*",
                    full.names = TRUE)
...
cib.tils %>%
  ...
  geom_violin(aes(fill = cancer), show.legend = FALSE) +
...
```{r}
write.csv(x = cib.tils,
          file = paste0("../data/split-deconvolved-aggcuff_absolute/",
                        format(Sys.Date(), "%F"), 
                        "_cibersort_absolute_aggregated+TILS.csv")
          )
```
