...
```{r}
all.nodes <- c(unique(weighted.graph$Gene), unique(weighted.graph$microbe))
all.nodes.combos <- combn(all.nodes, 2)
```
...
This takes about a minute, but there's ~ 34,000,000 rows. This needs to be a batch job.
```{r}
all.paths <- lapply(1:35, grabShortPaths)

saveRDS(all.nodes.combos,
        "/fs/ess/PAS1695/projects/exotic/data/network_all-node-combos.RDS")
saveRDS(weighted.edges, 
        "/fs/ess/PAS1695/projects/exotic/data/network_igraph-data.RDS")
```
